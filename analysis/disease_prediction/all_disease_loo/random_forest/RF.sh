#!/bin/bash
#SBATCH --job-name=rf
#SBATCH -N 1
#SBATCH -n 20
#SBATCH -p gpu
#SBATCH -w gpu01
#SBATCH --mem=50G
#SBATCH -o log/4_21_RF.out
#SBATCH -e log/4_21_RF.err
#SBATCH --exclude=cu01

source /home/cjj/miniconda3/etc/profile.d/conda.sh
conda activate microbiome_deep

script_path="shared/random_forest_model"
base_dir="data/disease_data"

# 遍历疾病目录
for disease_dir in "$base_dir"/*; do
    # 跳过非目录文件
    [ -d "$disease_dir" ] || continue
    
    disease=$(basename "$disease_dir")
    echo "Processing disease: $disease"
    

    # if [ "$disease" != "CRC" ] && [ "$disease" != "UC_biopsy" ] && [ "$disease" != "CD_biopsy" ] && [ "$disease" != "UC_feces" ] && [ "$disease" != "CD_feces" ] && [ "$disease" != "CD" ] && [ "$disease" != "UC" ] && [ "$disease" != "IBD_biopsy" ] && [ "$disease" != "CCD" ] && [ "$disease" != "IBD_feces" ] && [ "$disease" != "ICD" ]; then
    #     # 这行代码块只会在 disease 既不是 IBD 也不是 CRC 的时候执行
    #     echo "Skipping $disease as it is not IBD or CRC." # 示例输出
    #     continue
    # fi
    # if [ "$disease" != "UC_feces_to_biopsy" ] && [ "$disease" != "CD_feces_to_biopsy" ] && [ "$disease" != "IBD_feces_to_biopsy" ]; then
    #     # 这行代码块只会在 disease 既不是 IBD 也不是 CRC 的时候执行
    #     echo "Skipping $disease as it is not IBD or CRC." # 示例输出
    #     continue
    # fi

    # if  [ "$disease" != "IBD_feces_to_biopsy" ] && [ "$disease" != "CD_feces_to_biopsy" ] && [ "$disease" != "UC_feces_to_biopsy" ]; then
    #     # 这行代码块只会在 disease 既不是 IBD 也不是 CRC 的时候执行
    #     echo "Skipping $disease as it is not IBD or CRC." # 示例输出
    #     continue
    # fi
    if [ "$disease" != "AS" ] && [ "$disease" != "ASD" ] && [ "$disease" != "BD" ]  && [ "$disease" != "CAD" ] \
        && [ "$disease" != "GD" ] && [ "$disease" != "IBS" ] && [ "$disease" != "MS" ] && [ "$disease" != "OB" ] \
        && [ "$disease" != "PD" ] && [ "$disease" != "SZ" ] && [ "$disease" != "T2DM" ] && [ "$disease" != "IBD" ] && [ "$disease" != "CRC" ]; then
        # 这行代码块只会在 disease 既不是 IBD 也不是 CRC 的时候执行
        echo "Skipping $disease " # 示例输出
        continue
    fi
    # 获取研究列表
    studies=()
    while IFS= read -d $'\0' -r study; do
        studies+=("$(basename "$study")")
    done < <(find "$disease_dir" -mindepth 1 -maxdepth 1 -type d -print0)

    # 检查研究数量
    if [ ${#studies[@]} -le 1 ]; then
        echo "Skipping $disease (only ${#studies[@]} studies)"
        continue
    fi

    # 处理每个研究
    for study_id in "${studies[@]}"; do
        data_dir="$disease_dir/$study_id"
        result_dir="$data_dir/RF"
        
        # 创建结果目录
        if ! mkdir -p "$result_dir"; then
            echo "Failed to create directory: $result_dir"
            continue
        fi

        # 检查数据文件是否存在
        if [ ! -f "$data_dir/train_loo.biom" ] || [ ! -f "$data_dir/test_loo.biom" ]; then
            echo "Missing data files in $data_dir"
            continue
        fi

        echo "Processing $disease/$study_id"

        echo ${study_id} K_ >&2
        echo ${study_id} K_
        # 运行分类器
        python "$script_path/Randomforestclassifier.py" \
            "$data_dir/train_loo.biom" \
            "$data_dir/test_loo.biom" \
            "$disease_dir/metadata.tsv" \
            None \
            group \
            "$result_dir/loo.png" \
            "$result_dir/RF_ROC.csv" \
            "$result_dir/RF_Scores.csv"
    done
done
