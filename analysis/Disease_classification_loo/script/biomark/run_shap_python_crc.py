...
table_1 = pd.DataFrame(data=table_1, 
                    index=train_table.ids(axis='sample'),
...
X_valid = pd.DataFrame(data=X_valid, 
                       index=X_valid_table.ids(axis='sample'),
...
shap_table.to_csv(f"../../Data/biomark/shap_table_{CRC_study[i]}.csv", index=True)
