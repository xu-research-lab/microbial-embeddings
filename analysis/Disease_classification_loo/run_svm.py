...
# -*- coding: utf-8 -*-
"""
SVM across all LOSO tasks — simplest possible protocol.
...
For every fold:
  ...
  4. train one SVM on the whole training table, evaluate on the test table.
...
-----
  ...
  python run_svm.py --tasks all --top-k 600 --kernel rbf --C 1.0
  python run_svm.py --tasks all --top-k 0          # disable the filter
"""
...
MODEL_DIRNAME = "svm"          # sub-directory holding per-fold artifacts
...
def _parse_gamma(value):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            "--gamma must be 'scale', 'auto' or a float")
...
# =====================================================================
def run_job(job):
    """Train one SVM model on the full training table and write result.json."""
    ...
    try:
        ...
        svm = SVC(
            ...
            degree=job["degree"],
            coef0=job["coef0"],
            class_weight=job["class_weight"],
            probability=job["probability"],
            cache_size=job["cache_size"],
            max_iter=job["max_iter"],
            random_state=job["seed"])
        svm.fit(X_train, y_train)
        # AUC only needs a ranking, so the raw decision function is enough;
        # --probability turns on Platt scaling and uses predict_proba instead.
        if job["probability"]:
...
def build_jobs(task, args):
    ...
    for _, r in run.iterrows():
        ...
        jobs.append(dict(
            ...
            out_dir=os.path.join(t["artifact"], fold, MODEL_DIRNAME),
            ...
            probability=args.probability, cache_size=args.cache_size,
...
def main():
    ...
    ap.add_argument("--coef0", type=float, default=0.0,
                    help="poly / sigmoid kernel only")
    ap.add_argument("--class-weight", default=None,
                    choices=[None, "balanced"],
                    help="'balanced' re-weights classes by inverse frequency")
    ap.add_argument("--probability", action="store_true",
                    help="fit Platt scaling and score with predict_proba "
                         "(off by default: decision_function is used, which "
                         "gives the same AUC without an inner CV)")
    ap.add_argument("--cache-size", type=float, default=1000.0,
                    help="libsvm kernel cache in MB")
    ap.add_argument("--max-iter", type=int, default=-1,
                    help="libsvm iteration cap (-1 = no limit)")

    ap.add_argument("--n-jobs", type=int, default=10,
                    help="accepted for CLI compatibility; libsvm is "
    ...
    print(f"[cfg] kernel={args.kernel} C={args.C} gamma={args.gamma} "
          f"class_weight={args.class_weight} probability={args.probability} "
          f"seed={args.seed}")
