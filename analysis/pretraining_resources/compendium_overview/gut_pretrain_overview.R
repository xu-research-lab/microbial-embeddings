...
# load data
metadata <- read.csv("data/profile/meta_data_include_shannon.csv", header = TRUE)
...
### samples reads
df <- read.csv("data/profile/read_depth_samples.csv", header = FALSE)
colnames(df) <- "reads"
...
ggsave("results/figures/profile/read_depth_samples.pdf", p2, 
       width = 20, height = 18, units = "cm")


### prevalence distribution
prevalence <- read.csv("data/profile/prevalence.csv")
...
p3 <- ggplot(prevalence, aes(x = prevalence)) +
    ...
    theme(
        ...
        text = element_text(color = "black", face = "bold"),
        axis.text = element_text(color = "black"))
ggsave("results/figures/profile/prevalence.pdf", p3, 
       width = 20, height = 18, units = "cm")

### tax sample nums
final_df <- read.csv("data/profile/tax_num_samples.csv", row.names = 1)
taxonomy_mapping <- fromJSON("data/profile/taxonomy_mapping.json") 
...
phyla_abundance_top <- read.csv("data/profile/phyla_abundance_top.csv", row.names = 1)
...
p7 <- ggplot(hist_data, aes(x = bin_center, y = count, color = label)) +
    ...
    theme(text = element_text(face = "bold"), # 设置所有文本为加粗
          ...
          axis.title = element_text(face = "bold"), # 加粗坐标轴标题
          axis.text = element_text(face = "bold")) 
ggsave("results/figures/profile/top_phyla_abundance.pdf", p7, 
       width = 20, height = 18, units = "cm")

### shannon diversity
meta_data <- read.csv("data/profile/meta_data_include_shannon.csv")
...
ggsave("results/figures/profile/shannon.pdf", p8, 
       width = 20, height = 18, units = "cm")
...
braytsne <- read.csv("data/profile/bray_TSNE.csv",header = T)
metadata <- read.csv("data/profile/metadata_filter.txt" ,header = T,"\t")
...
ggsave("results/figures/profile/beta_sequence_region.pdf", combined_plot, 
       width = 22.5, height = 15, units = "cm")

# Load the data
pabundance <- read.csv("data/profile/phyla_abundance_top.csv", header = TRUE)
metadatashannon <- read.csv("data/profile/meta_data_include_shannon.csv", header = TRUE)
metadata <- read.csv("data/profile/metadata_filter.txt", header = TRUE,sep = "\t")
