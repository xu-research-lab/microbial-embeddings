...
for (i in 1: 10){
    ...
    file_dir <- sprintf("%s/%s/%s_100.txt", embedding_file,
                        methods[i], methods[i])
...
saveRDS(cor_list, sprintf("/home/dongbiao/word_embedding_microbiome/programe_test/phylogeny/pathway/cor_list_%s.rds", embedding_type))
...
ggsave(sprintf("/home/dongbiao/word_embedding_microbiome/programe_test/phylogeny/pathway/pathway_embedding_ratio_%s.png", 
               embedding_type), 
       p, width = 40, height = 25, units = 'cm', dpi = 300)
