...
base_data_dir="data/disease_data" # Input data directory (train/test/metadata)
embeding_base_dir="data/pretraining_datasize/embedding" # Base dir for datasize_result_* folders
result_base_dir="data/pretraining_datasize/result"   # Base dir for output results

target_diseases=("AS" "ASD" "BD" "CAD" "GD" "IBS" "MS" "OB" "PD" "SZ" "T2DM") # Add other diseases here if needed
...
for disease in "${target_diseases[@]}"; do
    ...
    for data_size_dir in "$embeding_base_dir"/datasize_result_*; do
        ...
        for embedding_file in "$data_size_dir"/subset_table_*.txt; do
            ...
            #     continue # Skip if not a valid file
            # fi
            if [ "$embedding_file" != "$data_size_dir/subset_table_2w_100.txt" ] && [ "$embedding_file" != "$data_size_dir/subset_table_4w_100.txt" ] && [ "$embedding_file" != "$data_size_dir/subset_table_8w_100.txt" ] && [ "$embedding_file" != "$data_size_dir/subset_table_1w_100.txt" ]; then
                continue # Skip if not a valid file
            fi
            ...
            for study_id in "${studies[@]}"; do
                ...
                (
                    ...
                    membed class-attention -g "$embedding_file" \
