...
#SBATCH -n 20       # 使用20核
#SBATCH --mem=100G  
#SBATCH -o log/4_21_attention_all_study.out  
#SBATCH -e log/4_21_attention_all_study.err  
...
for disease_dir in "$base_dir"/*; do
    ...
    for study_id in "${studies[@]}"; do
        ...
        if [ ${study_id} != "PRJEB31199" ] ; then
            echo "Skipping $study_id "
            continue
        fi
