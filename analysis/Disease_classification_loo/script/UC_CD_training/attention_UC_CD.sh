...
#SBATCH -n 20       # 使用20核
#SBATCH --mem=100G  
#SBATCH -o log/attention_UC_CD.out  
#SBATCH -e log/attention_UC_CD.err  
...
weight_decay=0.0001  
epoch=100
glove_embedding="filtered_p80_abundance_percentile_best"
...
for disease_dir in "$base_dir"/*; do
    ...
    if [ "$disease" != "UC" ] && [ "$disease" != "CD" ] && [ "$disease" != "UC_biopsy" ] && [ "$disease" != "CD_biopsy" ] && [ "$disease" != "UC_feces" ] && [ "$disease" != "CD_feces" ] && [ "$disease" != "IBD_biopsy" ] && [ "$disease" != "IBD_feces" ] && [ "$disease" != "CCD" ] && [ "$disease" != "ICD" ]; then
    ...
    for study_id in "${studies[@]}"; do
        ...
        (
            ...
            result_dir="$data_dir/results"  # 结果存储在study_id子目录下的results文件夹
            
            # 创建结果目录
            mkdir -p "$result_dir"

            membed class-attention -g "${glove_embedding}" \
