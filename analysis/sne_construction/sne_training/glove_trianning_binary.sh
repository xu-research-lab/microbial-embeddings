...
#SBATCH --job-name=glove_binary 
#SBATCH -N 1  
#SBATCH -p cu  
#SBATCH -n 28                 # 使用28核
#SBATCH --mem=250G  
#SBATCH -o master_%a_b.log  # 改动点
#SBATCH -e master_%a_b.err  # 改动点
#SBATCH --array=1-3
#SBATCH --exclude=cu01,cu[06-10]
...
process_percentile() {
  ...
  local percentile=$1
  local run=$2
  local work_dir="${BASE_DIR}/p${percentile}_${run}" # 改动点
